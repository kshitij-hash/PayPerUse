...
const ChatMessage: React.FC<ChatMessageProps> = ({ role, content, timestamp, paymentData }) => {
  ...
  const cardStyles = {
    user: 'bg-gradient-to-br from-purple-800/40 via-purple-900/30 to-pink-800/40 border border-purple-500/30 shadow-lg hover:shadow-purple-900/20 transition-all duration-300',
    assistant: 'bg-gradient-to-br from-black/60 via-purple-950/30 to-black/60 border-l-4 border-l-purple-500 border-t border-r border-b border-gray-800/50 shadow-lg hover:shadow-purple-900/10 transition-all duration-300',
    system: 'bg-gradient-to-r from-blue-900/30 to-indigo-900/20 border border-blue-800/50 text-blue-300 shadow-md',
  ...
  return (
    <div className={`flex items-start space-x-4 ${role === 'user' ? 'justify-end' : ''} mb-6`}>
      ...
      <div className={`flex-1 max-w-3xl ${role === 'user' ? 'text-right' : ''}`}>
          ...
          <CardContent className={`p-4 text-left ${role === 'assistant' ? 'bg-gradient-to-b from-black/0 to-purple-900/10' : ''} ${role === 'user' ? 'bg-gradient-to-b from-purple-900/5 to-pink-900/10' : ''}`}>
                ...
                components={{
                  ...
                  code({node, inline, className, children, ...props}: any) {
                    ...
                    return !inline && match ? (
                      ...
                      <code className="bg-gradient-to-r from-purple-900/40 to-pink-900/30 text-purple-200 px-1.5 py-0.5 rounded-md border border-purple-500/30 shadow-sm" {...props}>
                        {children}
                      </code>
                    );
                  }
            ...
            {paymentData && (
              <div className="mt-4 p-3 bg-gradient-to-r from-yellow-900/40 to-orange-900/40 border border-yellow-600/50 rounded-md backdrop-blur-sm shadow-lg transition-all duration-300 hover:shadow-orange-900/20">
                <h4 className="font-medium text-yellow-300 flex items-center"><AlertTriangle className="h-4 w-4 mr-2"/>Payment Required</h4>
                ...
                <div className="mt-3">
                  <Button 
                    ...
                    onClick={() => console.log('Payment flow triggered', paymentData)}
                  >
                    Pay Now
                  </Button>
                </div>
        ...
        <div className={`text-xs text-gray-500 mt-1.5 px-2 ${role === 'user' ? 'text-right' : 'text-left'} opacity-70 transition-opacity duration-300 hover:opacity-100`}>{formattedTime}</div>
      </div>
